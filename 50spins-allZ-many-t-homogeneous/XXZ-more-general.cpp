...
vector<vector<Eigen::Matrix2cd> > generate_W_state(){
    vector<vector<Eigen::Matrix2cd> > list_of_lists_rhoi;
        ...
        for(int Wj = 0; Wj < N; Wj++){
            ...
            for(int i = 0; i < N; i++){
                if(i == Wi && i == Wj)
                    ...
                    list_rhoi.push_back((Eigen::Matrix2cd() << 0, 0, 1, 0).finished());
                else if(i != Wi && i == Wj)
                    list_rhoi.push_back((Eigen::Matrix2cd() << 0, 1, 0, 0).finished());
                else
                    list_rhoi.push_back((Eigen::Matrix2cd() << 0, 0, 0, 1).finished());
            }
            list_of_lists_rhoi.push_back(list_rhoi);
        }
    }

    return list_of_lists_rhoi;
}

vector<vector<Eigen::Matrix2cd> > generate_blockW_state(){
    vector<vector<Eigen::Matrix2cd> > list_of_lists_rhoi;
    ...
    for(int Wi = 1; Wi < 9; Wi++){
        for(int Wj = 1; Wj < 9; Wj++){
            vector<Eigen::Matrix2cd> list_rhoi;
            for(int i = 0; i < N; i++){
                if(i / (N / 10) == Wi && i / (N / 10) == Wj)
                    list_rhoi.push_back((Eigen::Matrix2cd() << 1, 0, 0, 0).finished());
                else if(i / (N / 10) == Wi && i / (N / 10) != Wj)
                    list_rhoi.push_back((Eigen::Matrix2cd() << 0, 0, 1, 0).finished());
                else if(i / (N / 10) != Wi && i / (N / 10) == Wj)
                    list_rhoi.push_back((Eigen::Matrix2cd() << 0, 1, 0, 0).finished());
                else
...
vector<vector<Eigen::Matrix2cd> > generate_entangled_state(){
    vector<vector<Eigen::Matrix2cd> > four_lists_rhoi;

    double scale = 0.25;
    int oscillating_len = 25;
    int entangled_len = N - oscillating_len;
    ...
    for(int i = 0; i < N; i++){
        if(i < entangled_len){
            ...
            double cost = cos(M_PI/4 + (scale * M_PI * (i - entangled_len)));
            double sint = sin(M_PI/4 + (scale * M_PI * (i - entangled_len)));
            list_rhoi.push_back((Eigen::Matrix2cd() << cost * cost, cost * sint, cost * sint, sint * sint).finished());
        }
    }
    four_lists_rhoi.push_back(list_rhoi);
    ...
    return four_lists_rhoi;
}

vector<vector<Eigen::Matrix2cd> > generate_GHZ_neel_state(){
    vector<vector<Eigen::Matrix2cd> > four_lists_rhoi;
...
Eigen::MatrixXcd precompute_the_Z_parity(vector<Eigen::Matrix2cd> list_rhoi){
    ...
    Eigen::VectorXcd prefix_I(N);
    Eigen::VectorXcd suffix_I(N);

    prefix_I(0) = 1.0;
    for(int pos1 = 1; pos1 < N; pos1 ++)
        prefix_I(pos1) = prefix_I(pos1-1) * list_rhoi[pos1-1].trace();

    suffix_I(N-1) = 1.0;
    for(int pos2 = N-2; pos2 >= 0; pos2 --)
        suffix_I(pos2) = suffix_I(pos2+1) * list_rhoi[pos2+1].trace();

    for(int pos1 = 0; pos1 < N; pos1 ++){
        for(int pos2 = pos1; pos2 < N; pos2 ++){
            precomputed_Z(pos1, pos2) *= prefix_I(pos1) * suffix_I(pos2);
        }
    }
...
complex<double> product_state_exp_complex(vector<Eigen::Matrix2cd> list_rhoi, Eigen::MatrixXcd coef_c, int approx = -1){
    Eigen::MatrixXcd precomputed_Z = precompute_the_Z_parity(list_rhoi);
    ...
    for(int k = 0; k < 2 * N; k++){
        for(int l = 0; l < 2 * N; l++){
            ...
            complex<double> val_kl = coef_c(k, l) * product_state_exp_fermionic_oper(list_rhoi, k, l, precomputed_Z);
            val += val_kl;
        }
    }
...
int main(){
    ...
    // cout << "Field: ";
    // for(int i = 0; i < N; i ++){
    //     double val = x(random_vector()) / 200;
    //     field_h.push_back( val ); // between [-5, 5]
    //     cout << val << ", ";
    // }
    // cout << endl;
    ...
    double t_ls[6] = {0.0, 1.0, 4.0, 10.0, 1000.0, 1000000.0};
    //
    // cout << "All0 ";
    // cout << "[";
    // for(auto t : t_ls){
    //     for(int j = 0; j < N; j++){
    //         double val = product_state_exp(generate_all_zero_state(), Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
    //         cout << val * 2 - 1 << "," << flush;
    //     }
    // }
    // cout << "]" << endl;
    ...
    // for(auto t : t_ls){
    //     for(int j = 0; j < N; j++){
    //         double val = product_state_exp(generate_half_half_state(), Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
    //         cout << val * 2 - 1 << "," << flush;
    //     }
    // }
    // cout << "]" << endl;
    ...
    // for(auto t : t_ls){
    //     for(int j = 0; j < N; j++){
    //         double val = product_state_exp(generate_neel_state(), Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
    //         cout << val * 2 - 1 << "," << flush;
    //     }
    // }
    // cout << "]" << endl;
    ...
    for(auto t : t_ls){
        for(int j = 0; j < N; j++){
            vector<vector<Eigen::Matrix2cd> > four_lists_rhoi = generate_entangled_state();
            complex<double> val = 0;
            for(int k = 0; k < 4; k++)
                val += product_state_exp_complex(four_lists_rhoi[k], Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
            assert(abs(val.imag()) < 1e-7);
            cout << val.real() - 1 << "," << flush;
        }
    }
    cout << "]" << endl;
    ...
    // for(auto t : t_ls){
    //     for(int j = 0; j < N; j++){
    //         vector<vector<Eigen::Matrix2cd> > list_of_lists_rhoi = generate_blockW_state();
    //         complex<double> val = 0;
    //         for(int k = 0; k < (int)list_of_lists_rhoi.size(); k++)
    //             val += product_state_exp_complex(list_of_lists_rhoi[k], Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
    //         assert(abs(val.imag()) < 1e-7);
    //         cout << 2 * val.real() / 8 - 1 << "," << flush;
    //     }
    // }
    // cout << "]" << endl;
    //
    ...
    //     for(auto t : t_ls){
    //         for(int j = 0; j < N; j++){
    //             double val = product_state_exp(state, Heisenberg_Evolved_ai_dagger_ai(j, t, c_in_eta, eta_in_c, Energy));
    //             valFile << val << ",";
    //         }
    //     }
    //     valFile << "]" << endl;
